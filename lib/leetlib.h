...
void DrawSprite(void *sprite, int xcentre, int ycentre, int xsize, int ysize, float rotate_angle_radians=0, DWORD tint_col_argb = 0xffffffff);
...
struct Bullet
{
	...
	float BA;
	bool active;
	Bullet() : BX(0), BY(0), BA(0.0f), active(false) {}
};
...
bool renderEnemies(std::vector<Enemy>& enemies, std::vector<Bullet>& bullets,
	void* enemySprite, const Hitbox& playerHitbox, int time,
	int& startTimer, bool& gameover, unsigned int& score);